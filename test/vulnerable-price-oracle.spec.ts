...
describe('Oracle manipulation', function () {
    it('should succeed in exploiting a vulnerable price oracle to make a profit', async function () {
        // Init stuff
        ...
        const VulnerableLendingProtocol = await ethers.getContractFactory(
            'VulnerableLendingProtocol'
        )
        ...
        const simpleOracleAttack = await SimpleOracleAttack.deploy(
        ...
        expect(resultingDai.gt(startingDai)).to.equal(true)
    })
})
