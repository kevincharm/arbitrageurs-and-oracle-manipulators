...
describe('Frontrun', function () {
    it('should perform successful sandwich attack', async function () {
        ...
        const amountsOut = (
            await uniswapV2Router.functions.getAmountsOut(amountEthToSwap, [
                wethAddress,
                daiAddress,
            ])
        )[0]
        ...
        console.log(
            `Quote: ${ethers.utils
                .formatEther(inAmount.toString())
                .toString()} ETH -> ${ethers.utils
        ...
        for (const txHash of pendingBlock.transactions as string[]) {
            ...
            const amountsOut = (
                await uniswapV2Router.functions.getAmountsOut(userWethValue, [
                    wethAddress,
                    daiAddress,
                ])
            )[0]
            ...
            const [daiReserve, wethReserve] =
                (await uniswapV2DaiEthPair.functions.getReserves()) as [BigNumber, BigNumber]
            console.log(
            ...
            const frontrunTx = await uniswapV2Router.populateTransaction.swapExactETHForTokens(
                userMinDaiValue,
                [wethAddress, daiAddress],
                attacker.address,
                deadline,
            ...
            const frontrunTxResult = await attacker.sendTransaction(frontrunTx)
            console.log(`Sent frontrun pump tx ${frontrunTxResult.hash} from ${attacker.address}!`)

            const amountDaiSell = userMinDaiValue // TODO: Get actual amount
            const backrunTx = await uniswapV2Router.populateTransaction.swapExactTokensForETH(
                amountDaiSell,
            ...
            const backrunTxResult = await attacker.sendTransaction(backrunTx)
            console.log(`Sent frontrun dump tx ${backrunTxResult.hash} from ${attacker.address}!`)
            ...
            const blockTxes = pendingBlockWithInsertedTxes.transactions as string[]
            // Assert that we have guaranteed the transaction order!
            expect(blockTxes.indexOf(frontrunTxResult.hash)).to.equal(blockTxes.indexOf(txHash) - 1)
            expect(blockTxes.indexOf(backrunTxResult.hash)).to.equal(blockTxes.indexOf(txHash) + 1)
