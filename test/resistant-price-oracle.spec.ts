...
describe('Oracle manipulation', function () {
    it('should fail when trying to manipulate a resistant price oracle', async function () {
        // Init stuff
        const signers = await ethers.getSigners()
        const signer = signers[0] // Loaded with 10k ETH

        // Deploy TWAP price oracle (must be first)
        const PriceOracle = await ethers.getContractFactory('DaiWethTwapPriceOracle')
        const priceOracle = await PriceOracle.deploy()
        await Promise.all([mine(), priceOracle.deployed()])
        console.log((await priceOracle.functions.getEthTwapPrice())[0].toString())
        // Deploy force sender
        ...
        const ResistantLendingProtocol = await ethers.getContractFactory('ResistantLendingProtocol')
        const resistantLendingProtocol = await ResistantLendingProtocol.deploy(priceOracle.address)
        await Promise.all([mine(), forceSendEther.deployed(), resistantLendingProtocol.deployed()])
        // Deploy attacker contract
        const SimpleOracleAttack = await ethers.getContractFactory('SimpleOracleAttack')
        const simpleOracleAttack = await SimpleOracleAttack.deploy(resistantLendingProtocol.address)
        ...
        let mintDaiTx = await daiSigner.sendTransaction(
            await Dai.populateTransaction.mint(
                resistantLendingProtocol.address,
        ...
        await mine()
        // Expect that this will fail because the price oracle is a TWAP oracle!
        await expect(attackTx.wait()).to.be.rejectedWith(Error)
